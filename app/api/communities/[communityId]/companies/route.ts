...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ communityId: string }> | { communityId: string } }
) {
  try {
    await connectDB();
...
export async function DELETE(
  ...
  try {
    ...
    community.companies = community.companies.filter(
      (c: any) => {
        if (typeof c === 'string') {
          return c.trim().toLowerCase() !== company.name.trim().toLowerCase();
        }
        return c.toString() !== company._id.toString();
      }
    );
    await community.save();
